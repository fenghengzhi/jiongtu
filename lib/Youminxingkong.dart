...
class Youminxingkong extends BaseList {
  ...
  Future<List<Resource>> getListData() async {
    final response = await http.get('https://www.gamersky.com/ent/147/');
    if (response.statusCode == 200) {
      ...
      final resources = list.map((item) {
        ...
        final exp = new RegExp(
            r"(?<=^https:\/\/www\.gamersky\.com\/ent\/\d*\/)(\d*)(?=.shtml$)");
        // debugger();
        return Resource(
            title: item.querySelector('.tc').text,
            coverUrl: item.querySelector('.img .pe_u_thumb').attributes['src'],
  ...
  Future<List<PicInfo>> getItemData(Resource resource) async {
    final response =
        await http.post('http://appapi2.gamersky.com/v5/getArticle',
            ...
            if (img != null) {
              ...
              if (img.attributes['original-image-width'] != null &&
                ...
                return PicInfo(
                  ...
                  width: int.parse(img.attributes['original-image-width']),
                  height: int.parse(img.attributes['original-image-height']),
                );
              } else {
                return PicInfo(
                  title: box.text,
